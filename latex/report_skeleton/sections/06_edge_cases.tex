...
The \texttt{low\_card} config (5 rows/entity, 100 entities, 10\% overlap) exposes fundamental failure modes:
...
When $m,n$ are large (e.g., 100 $\times$ 120 = 12,000 pairs per global entity), the Cartesian product explodes.
Blocking must be aggressive, but aggressive blocking loses recall.
The \texttt{c2} config (200/200) has 40,000 pairs per global entity—computationally prohibitive without strong blocking.
...
As we see in \ref{sec:exp_aggregation}, single scalar aggregation handles all cardinality regimes robustly.
...
% 	\item Profile construction: Aggregator runs on 5-record groups; vectors are sparse but informative.
